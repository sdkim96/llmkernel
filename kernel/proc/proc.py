...
class Process:
    ...
    async def run(self) -> t.AsyncGenerator[Event, None]:
        ...
        while self.pcb.state != ProcessState.TERMINATED:
                decision = await self.kernel._call_cpu(pid=self.pcb.pid)
    ...
    async def call_agent(
        ...
        *,
        ...
        await self.kernel.syscall(type="call_agent", id=agent_id, kwargs={})
    async def call_tool(
        self,
        *,
        tool_id: str,
    ): 
        await self.kernel.syscall(type="call_tool", id=tool_id, kwargs={})
